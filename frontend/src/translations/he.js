export const he = {
  common: {
    all: "הכל",
    yes: "כן",
    no: "לא",
    none: "—",
    save: "שמירה",
    cancel: "ביטול",
    close: "סגירה",
    confirm: "אישור",
    delete: "מחיקה",
    edit: "עריכה",
    add: "הוספה",
    search: "חיפוש",
    back: "חזרה",
    next: "הבא",
    loading: "טוען...",
    required: "שדה חובה",
    success: "הפעולה הצליחה",
    error: "אירעה שגיאה",
    monthFilter: {
      allTime: "כל הזמנים",
      allMonthsInYear: "כל החודשים",
      yearLabel: "סינון לפי שנה",
      monthLabel: "סינון לפי חודש",
    },
    loadMore: "טען עוד ({count})",
    errorBoundaryTitle: "לא ניתן להציג את החלק הזה",
    errorBoundaryMessage: "כדאי לרענן את העמוד, או לחזור אחורה ולנסות שוב.",
  },

  manager: {
    nav: {
      roleMain: "מנהל ראשי",
      sectionMain: "ראשי",
      overview: "סקירה כללית",
      inventory: "ניהול מלאי",
      alerts: "התראות",
      sectionSales: "מכירות",
      customerOrders: "הזמנות לקוחות",
      giftCardOrders: "כרטיסי מתנה",
      deliveryTracking: "מעקב הזמנות",
      salesReceipts: "קבלות מכירה",
      analytics: "אנליטיקה",
      customerFeedback: "משוב לקוחות",
      stockRequests: "בקשות מלאי",
      returnRequests: "בקשות החזרה",
      contactMessages: "הודעות יצירת קשר",
      coupons: "קופונים",
      sectionSettings: "הגדרות",
      settings: "הגדרות",
      lightMode: "מצב בהיר",
      darkMode: "מצב כהה",
      logout: "התנתק",
    },
    topbar: {
      searchPlaceholder: "חיפוש לפי שם/ברקוד",
      backTitle: "חזרה",
      scanTitle: "סריקה",
      menuTitle: "תפריט",
      cancelPromoTitle: "בטל פרסום פעיל",
      promoActiveButton: "📢 פרסום פעיל — בטל",
      refreshTitle: "🔄רענן",
      newProductButton: "+ מוצר חדש",
    },
    overview: {
      greeting: "שלום 👋",
      connected: "● מקוון",
      stockItems: "פריטים במלאי",
      productsSuffix: "מוצרים",
      sales: "מכירות החודש",
      transactionsSuffix: "עסקאות החודש",
      stockIssues: "מוצרים בבעיית מלאי",
      needsAttention: "אזלו או מלאי נמוך",
      highDemand: "ביקושים גבוהים",
      activeAlerts: "🔔 התראות פעילות",
      seeAll: "הכל ←",
      noActiveAlerts: "✅ אין התראות פעילות",
      weeklySales: "📊 מכירות שבועיות",
      perDay: "₪ לפי יום",
      slowMovingTitle: "📦 מוצרים שלא נמכרים — מועמדים לפרסום",
      slowMovingTag: "הכי פחות נמכרים",
      allSellingWell: "✅ כל המוצרים נמכרים כראוי!",
      salesCount: "מכירות",
      promoted: "✅ בפרסום",
      promoteButton: "📢 פרסם",
    },
    orders: {
      title: "הזמנות לקוחות",
      subtitle: "הזמנות שנפתחו — יש לאשר ולעקוב",
      pendingConfirmation: "ממתינות לאישור",
      toHandle: "לטיפול",
      total: 'סה"כ',
      ordersSuffix: "הזמנות",
      confirmed: "מאושרות",
      handled: "טופלו",
      searchPlaceholder: "🔍 חיפוש לפי טלפון או מספר הזמנה \u200e(RCP-...)\u200e",
      allMonths: "📅 כל החודשים",
      noOrdersFiltered: "אין הזמנות תואמות לסינון",
      noOrdersOpen: "אין הזמנות פתוחות",
      orderLabel: "📦 הזמנה",
      statusConfirmed: "✅ אושרה",
      statusPending: "⏳ ממתינה לאישור",
      customSizeTag: "⚠️ מידה מיוחדת",
      confirmOrderButton: "✅ אישור הזמנה",
      rejectOrderButton: "✕ דחיית הזמנה",
      confirmRejectPrompt: "לדחות את ההזמנה? תישלח על כך הודעה במייל.",
      confirmApprovePrompt: "לאשר את ההזמנה? תישלח על כך הודעה במייל.",
      confirmApproveGiftCardPrompt:
        "לאשר את ההזמנה? כרטיס המתנה יופעל, ותישלח על כך הודעה במייל.",
      decisionWriteFailed:
        "הפעולה לא נשמרה. ההזמנה נשארה כפי שהייתה, ואפשר לנסות שוב.",
      decisionEmailFailed:
        "ההזמנה עודכנה, אבל ההודעה במייל לא נשלחה. כדאי ליצור קשר ישירות.",
      decisionGiftCardFailed:
        "ההזמנה עודכנה, אבל כרטיס המתנה לא עודכן. כדאי לבדוק אותו במסך כרטיסי המתנה.",
      orderDetailsButton: "📋 פרטי הזמנה",
      cancelledLabel: "✕ בוטלה",
      cancelledSuffix: "בוטלו",
      rejectedLabel: "🚫 נדחתה",
      rejectedSuffix: "נדחו",
      overdueTag: "⏰ מתעכבת",
    },
    giftCardOrders: {
      title: "🎁 הזמנות כרטיסי מתנה",
      subtitle: "מעקב אחר כל כרטיסי המתנה שנרכשו בחנות",
      totalSold: "סה\"כ נמכרו",
      cardsSuffix: "כרטיסים",
      totalValue: "שווי כולל",
      allAmounts: "💰 כל הסכומים",
      amountUnder100: "עד ₪100",
      amount100to300: "₪100–300",
      amountOver300: "מעל ₪300",
      allMonths: "📅 כל החודשים",
      searchPlaceholder: "🔍 חיפוש לפי קוד או שם מקבל",
      noCards: "אין כרטיסי מתנה תואמים",
      codeLabel: "קוד",
      recipientLabel: "מקבל/ת",
      messageLabel: "הודעה",
      buyerLabel: "נרכש על ידי",
      amountLabel: "סכום",
      dateLabel: "תאריך רכישה",
      noMessage: "—",
      statusActive: "✓ פעיל",
      statusUsed: "✕ נוצל במלואו",
      statusPending: "⏳ ממתין לאישור",
      statusRejected: "🚫 נדחה",
      balanceLabel: "יתרה נוכחית",
      openBalance: "יתרה פתוחה",
      openBalanceSub: "נכון להיום",
    },
    deliveries: {
      title: "מעקב הזמנות",
      subtitle: "עדכון סטטוס הזמנות",
      allTab: "🔔 הכל",
      allMonths: "📅 כל החודשים",
      noOrdersInStage: "אין הזמנות בשלב הזה",
      defaultCustomer: "לקוח",
      sizeLabel: "מידה:",
      qtyLabel: "כמות:",
      updateToPrefix: "✓ עדכן ל:",
      selfPickupBadge: "איסוף עצמי",
      itemsCountSuffix: "פריטים בהזמנה",
      orderDetailsButton: "📋 פרטי הזמנה",
      overdueTag: "⏰ מתעכבת",
    },
    coupons: {
      title: "🎟️ ניהול קופונים",
      subtitle: "הוספה, הפעלה/כיבוי ומעקב שימוש בקופונים בזמן אמת",
      newCouponTitle: "➕ קופון חדש",
      codeLabel: "קוד קופון",
      codePlaceholder: "לדוגמה: WINTER10",
      discountPercentLabel: "אחוז הנחה",
      seasonRestrictionLabel: "הגבלת עונה",
      seasonAllYear: "כל השנה (ללא הגבלה)",
      seasonSummer: "קיץ בלבד",
      seasonWinter: "חורף בלבד",
      seasonSpringAutumn: "אביב/סתיו בלבד",
      addCouponButton: "הוספת קופון",
      noCouponsYet: "עדיין אין קופונים",
      discountSuffix: "% הנחה",
      validOnlyIn: " · תקף רק ב-",
      usedTimes: "📊 נעשה בו שימוש {count} פעמים",
      active: "✓ פעיל",
      inactive: "✕ כבוי",
      disableButton: "כבה",
      enableButton: "הפעל",
      codeRequired: "יש להזין קוד קופון",
      codeExists: "קוד הקופון הזה כבר קיים",
      confirmDeleteCoupon: "למחוק את הקופון {code}?",
    },
    stockNotifications: {
      title: 'בקשות "הודע לי כשחוזר למלאי"',
      subtitle: '{pending} בקשות ממתינות מתוך {total} סה"כ',
      infoNote: 'המייל וההתראה נשלחים אוטומטית ברגע שמעדכנים את כמות המלאי ב"פרטים" ל-0 ומעלה',
      noRequestsYet: "עדיין אין בקשות",
      confirmDeleteRequest: "למחוק את הבקשה?",
      autoHandleTooltip: "יטופל אוטומטית כשהמלאי יעודכן",
      pendingStock: "⏳ ממתין לעדכון מלאי",
      notified: "✓ הודע",
      filterAll: "🔔 הכל",
      filterPending: "⏳ ממתין",
      filterNotified: "✓ טופל",
      allMonths: "📅 כל החודשים",
      filteredByProduct: "מסונן למוצר:",
    },
    returns: {
      title: "בקשות החזרה וזיכוי",
      subtitle: "ניהול בקשות החזרה ומעקב סטטוס",
      filterAll: "🔔 הכל",
      filterPending: "⏳ ממתין",
      filterApproved: "✅ אושר",
      filterRejected: "❌ נדחה",
      allMonths: "📅 כל החודשים",
      noRequestsYet: "עדיין אין בקשות החזרה",
      orderLabel: "הזמנה:",
      reasonLabel: "סיבה:",
      noteLabel: "הערה:",
      statusPending: "⏳ ממתין לטיפול",
      statusApproved: "✅ אושר",
      statusRejected: "❌ נדחה",
      approveButton: "✅ אישור החזרה",
      rejectButton: "❌ דחיית בקשה",
      reasonDefective: "מוצר פגום",
      reasonWrongSize: "מידה לא מתאימה",
      reasonNotAsDescribed: "המוצר לא תואם לתיאור",
      reasonChangedMind: "שיניתי את דעתי",
      reasonOther: "אחר",
      noRestockNote: "לא יוחזר למלאי — מוצר פגום",
      creditMessage: "זיכוי אוטומטי עבור החזרת {item}",
      creditFallbackItem: "פריט",
    },
    contactMessages: {
      title: "הודעות יצירת קשר",
      subtitle: "הודעות שנשלחו מלקוחות דרך עמוד המדיניות",
      filterAll: "🔔 הכל",
      filterUnread: "🔵 לא נקרא",
      filterRead: "✓ נקרא",
      allMonths: "📅 כל החודשים",
      noMessagesYet: "עדיין אין הודעות",
      anonymous: "לקוח/ה",
      markAsRead: "✓ סמן כנקרא",
      replyLabel: "תשובה ללקוח/ה",
      replyPlaceholder: "כתיבת תשובה שתישלח במייל...",
      replySendButton: "שליחת תשובה",
      replySending: "שולח...",
      replyFailed: "⚠️ התשובה לא נשלחה. אפשר לנסות שוב בעוד רגע.",
      replyNoAddress: "לפנייה הזו לא צורפה כתובת מייל, ולכן אי אפשר להשיב מכאן.",
      repliedLabel: "✓ נענתה",
    },
    feedback: {
      title: "משוב לקוחות",
      subtitle: "מה לקוחות כתבו לפני התשלום",
      noFeedbackYet: "עדיין אין משובים",
      noRating: "ללא דירוג",
      filterAll: "🔔 הכל",
      filterUnread: "🔵 לא נקרא",
      filterRead: "✓ נקרא",
      allMonths: "📅 כל החודשים",
      markAsRead: "✓ סמן כנקרא",
      markAsUnread: "↺ סמן כלא נקרא",
      alreadyRead: "✓ נקרא",
      unreadBadge: "חדש",

      // Labels for the topics older feedback was tagged with. The customer
      // form no longer offers them, but entries from when it did still need
      // naming, so they live with the screen that displays that history.
      topicDesign: "🎨 עיצוב",
      topicSearch: "🔍 חיפוש",
      topicShopping: "🛒 קנייה",
      topicMobile: "📱 מובייל",
      topicChatbot: "💬 צ׳אטבוט",
      topicSuggestion: "💡 הצעה",
    },
    analytics: {
      title: "אנליטיקה",
      subtitle: "סטטיסטיקות מכירות אמיתיות, מחושבות מההזמנות בפועל",
      otherCategory: "אחר",
      monthRevenue: "הכנסות החודש",
      monthSales: "מכירות החודש",
      avgOrder: "ממוצע עסקה",
      repeatCustomers: "לקוחות חוזרים",
      repeatAllTime: "מכל התקופה",
      repeatFraction: "{repeat} מתוך {total}",
      monthExpenses: "הוצאות החודש",
      monthProfit: "רווח החודש",
      salesByCategory: "מכירות לפי קטגוריה — החודש",
      noSalesThisMonth: "עדיין אין מכירות החודש",
      noteTitle: "💡 הערה",
      missingCostNote:
        '{count} פריטים שנמכרו החודש נמכרו ממוצר בלי "עלות מוצר" רשומה — עבורם ההוצאה נספרה כ-₪0, כך שההוצאות/הרווח בפועל עשויים להיות שונים. אפשר להוסיף עלות לכל מוצר דרך "פרטים" בניהול מלאי.',
      returnsNoteTitle: "🔄 החזרות שהופחתו החודש",
      returnsNote: "{count} החזרות מאושרות החודש הופחתו מההכנסה, בסך כולל של ₪{amount}.",
    },
    receipts: {
      title: "קבלות מכירה",
      subtitle: "חיפוש לפי קוד קבלה",
      searchTitle: "🔍 חיפוש לפי קוד קבלה",
      searchPlaceholder: "קוד קבלה",
      searchButton: "חיפוש",
      enterCodeToSearch: "קוד קבלה לחיפוש",
      notFound: "❌ לא נמצאה קבלה:",
      foundReceipts: "✅ נמצאו {count} קבלות",
      allRecentReceipts: "📋 כל הקבלות האחרונות",
      receiptDetailsButton: "📋 פרטי קבלה",
      filterByMonthLabel: "סינון לפי חודש",
      allMonths: "📅 כל החודשים",
    },
    receiptDetailsModal: {
      title: "פרטי קבלת מכירה",
      unknown: "לא ידוע",
      customerDetailsTitle: "👤 פרטי לקוח",
      fullName: "שם מלא:",
      email: "✉️ מייל:",
      phone: "📞 טלפון:",
      address: "🏠 כתובת:",
      itemsTitle: "🛍️ פריטים",
      qtyLabel: "כמות:",
      totalLabel: 'סה"כ לתשלום',
      paymentMethod: "אמצעי תשלום:",
      payCard: "כרטיס אשראי",
      payCash: "מזומן",
      payGiftCard: "כרטיס מתנה",
      close: "סגירה",
    },
    settings: {
      title: "⚙️ הגדרות מערכת",
      subtitle: "ניהול חנות והגדרות כלליות",
      storeDetailsTitle: "🏪 פרטי החנות",
      hoursSectionTitle: "🕒 ימי ושעות פעילות",
      dayNames: {
        sun: "א׳",
        mon: "ב׳",
        tue: "ג׳",
        wed: "ד׳",
        thu: "ה׳",
        fri: "ו׳",
        sat: "שבת",
      },
      openTimeLabel: "שעת פתיחה",
      dayClosed: "סגור",
      closeTimeLabel: "שעת סגירה",
      saveHoursButton: "💾 שמירת שעות פעילות",
      saveFailed: "⚠️ השמירה נכשלה. כדאי לבדוק את החיבור ולנסות שוב.",
      hoursErrors: {
        closeBeforeOpen: "ב{day}: שעת הסגירה ({close}) חייבת להיות אחרי שעת הפתיחה ({open}).",
        invalidTime: "ב{day}: שעה לא תקינה. יש להזין שעה בין 00:00 ל-23:59.",
      },
      hoursSaved: "✅ שעות הפעילות נשמרו",
      policySectionTitle: "📋 תוכן עמוד המדיניות",
      policyReturnsLabel: "מדיניות החזרות",
      policyCancellationLabel: "מדיניות ביטול הזמנה",
      policyAboutStoreLabel: "אודות החנות",
      policyAboutStoreHint: "השתמשי ב-{address} במקום שבו רוצים שהכתובת האמיתית (מהפרטים למעלה) תופיע אוטומטית.",
      policyShippingLabel: "מדיניות משלוחים (4 שורות)",
      policyPrivacyLabel: "מדיניות פרטיות",
      policySaveButton: "💾 שמירת תוכן מדיניות",
      policyTranslateButton: "🌐 תרגם לאנגלית ושמור",
      policyTranslatingButton: "מתרגם…",
      policySavingButton: "⏳ שומר...",
      policySaved: "✅ תוכן המדיניות נשמר ותורגם",
      policyEnglishTitle: "🌐 תוכן באנגלית",
      storeName: "שם החנות",
      phone: "טלפון",
      email: "אימייל",
      address: "כתובת",
      addressEnLabel: "כתובת (אנגלית)",
      saveDetails: "💾 שמירת פרטים",
      detailsSaved: "✅ פרטים נשמרו",
      notificationSettingsTitle: "🔔 הגדרות התראות",
      lowStockLabel: "התראות מלאי נמוך",
      lowStockDesc: "קבל התראה כשמוצר מתחת לרף המינימום",
      outOfStockLabel: "התראות מלאי אפס",
      outOfStockDesc: "קבל התראה כשמוצר אוזל לחלוטין",
      highDemandLabel: "התראות ביקושים גבוהים",
      highDemandDesc: "מוצרים עם יותר מ-{count} בקשות התראה",
      demandThresholdLabel: "סף ביקושים לאזהרה",
      demandThresholdDesc: "כמות בקשות מינימלית להתראה",
      saveSettings: "💾 שמירת הגדרות",
      settingsSaved: "✅ הגדרות נשמרו",
      historicalTranslationTitle: "🌍 עדכון תרגומים היסטורי",
      historicalTranslationDesc: "מתרגם לאנגלית הזמנות, פניות, משוב, לקוחות, החזרות והתראות מלאי שעדיין לא תורגמו — כתובות, שמות וכרטיסי מתנה כלולים.",
      historicalTranslationButton: "עדכן נתונים היסטוריים",
      historicalTranslationRunning: "מתרגם... ({done}/{total})",
      historicalTranslationDone: "✅ הושלם! כל הנתונים עודכנו.",
      historicalTranslationNothing: "✅ הכול מתורגם, אין מה לעדכן",
      historicalTranslationPartial: "⚠️ הסתיים, אך {failed} פריטים לא עודכנו. אפשר להריץ שוב.",
      failedTranslationsFound: "⚠️ נמצאו {count} פריטים שהתרגום שלהם לאנגלית נכשל או חסר",
    },
    orderDetailsModal: {
      unknown: "לא ידוע",
      notEntered: "לא הוזן",
      payCard: "כרטיס אשראי",
      payCash: "מזומן",
      payGiftCard: "כרטיס מתנה",
      title: "📋 פרטי הזמנה",
      statusLabel: "🚦 סטטוס:",
      statusReady: "מוכן להגשה",
      statusPending: "ממתין להכנה",
      dateLabel: "🕒 תאריך:",
      customerDetailsTitle: "👤 פרטי לקוח",
      fullName: "שם מלא:",
      phone: "📞 טלפון:",
      email: "✉️ מייל:",
      city: "🏙️ עיר:",
      street: "🏠 רחוב:",
      zip: "📮 מיקוד:",
      notes: "📝 הערות:",
      itemsTitle: "🛍️ פריטים",
      sizeLabel: "מידה:",
      qtyLabel: "כמות:",
      customSizeWarning: '⚠️ בקשת מידה מיוחדת ("{size}") — דורש אישור ידני לפני משלוח',
      shippingPaymentTitle: "🚚 משלוח ותשלום",
      shippingMethod: "שיטת משלוח:",
      shippingNameStandard: "משלוח רגיל",
      shippingNameExpress: "משלוח מהיר",
      shippingNameSameDay: "משלוח באותו יום",
      shippingNamePickup: "איסוף עצמי",
      paymentMethod: "אמצעי תשלום:",
      installmentsCount: "מספר תשלומים:",
      discount: "הנחה:",
      pointsRedeemed: "נוצלו {points} נקודות נאמנות (−₪{amount})",
      totalToPayLabel: '💰 סה"כ לתשלום:',
      close: "סגירה",
      checkCodePlaceholder: "קוד כרטיס מתנה/זיכוי (GC-... או RTN-...)",
    },
    scanModal: {
      title: "📷 סריקת ברקוד",
      loadingCamera: "טוען מצלמה...",
      searchButton: "חיפוש",
      cameraError: "שגיאה בגישה למצלמה",
      searchingBarcode: "🔍 מחפש ברקוד...",
      initializingCamera: "🔍 מאתחל מצלמה...",
      cameraPermissionNeeded: "⚠️ יש לאשר גישה למצלמה",
      scanning: "🔍 סורק...",
      notDetected: "❌ לא זוהה — אפשר לנסות שוב",
      subtitle: "סרוק ברקוד לעדכון מלאי",
      cameraTab: "📹 מצלמה",
      manualTab: "⌨️ ידנית",
      scanningButton: "⏳ סורק...",
      clickToScan: "לסריקה",
      manualInputLabel: "קוד מוצר ידני:",
      manualPlaceholder: "לדוגמה: FS-001",
    },
    promoModal: {
      title: "📢 הפעלת פרסומת למוצר",
      salesCount: "מכירות",
      currentlyActive: "✅ פרסומת פעילה כרגע",
      cancelPromo: "❌ בטל פרסומת",
      activateButton: "🚀 הפעל פרסומת",
      cancelButton: "ביטול",
    },
    addProductModal: {
      cameraAccessError: "לא ניתן לגשת למצלמה",
      selectImageFile: "⚠️ יש לבחור קובץ תמונה",
      fillAllFields: "❌ יש למלא את כל השדות ולהוסיף תמונה",
      codeAlreadyExists: "❌ הקוד {code} כבר קיים במלאי — כל מוצר חייב קוד ייחודי",
      duplicateColor: "❌ יש כאן שני צבעים עם אותו שם — כל צבע צריך שם ייחודי",
      title: "מוצר חדש",
      codeLabel: "קוד",
      scanTooltip: "סרוק ברקוד",
      nameLabel: "שם",
      namePlaceholder: "שם המוצר",
      categoryLabel: "קטגוריה",
      genderLabel: "מגדר",
      seasonLabel: "עונה",
      chooseSeasonPlaceholder: "עונה...",
      seasonAllYear: "כל השנה",
      seasonSummer: "קיץ",
      seasonWinter: "חורף",
      seasonSpringAutumn: "אביב-סתיו",
      quantityLabel: "כמות",
      priceLabel: "מחיר (₪)",
      costLabel: "עלות מוצר (₪)",
      costPlaceholder: "כמה עולה לכן לייצר/לקנות את הפריט",
      minStockLabel: "מינימום להתראה",
      variantTotal: 'סה״כ: {total} יח׳',
      colorNamePlaceholder: "שם הצבע",
      addColorBreakdown: "➕ הוספת פילוח לפי צבע",
      descriptionLabel: "תיאור",
      descriptionPlaceholder: "תיאור קצר...",
      productImageLabel: "תמונת מוצר",
      fileTab: "📁 קובץ",
      cameraTab: "📸 מצלמה",
      previewAlt: "תצוגה מקדימה",
      removeImage: "🗑️ הסר תמונה",
      clickToUpload: "להעלאה",
      openCamera: "📷 פתיחת מצלמה",
      addProductButton: "הוספת מוצר",
      translatingButton: "⏳ מתרגם לאנגלית...",
    },
    detailsModal: {
      seasonSummer: "קיץ",
      seasonWinter: "חורף",
      seasonSpringAutumn: "אביב/סתיו",
      seasonAllYear: "כל השנה",
      duplicateColor: "יש כאן שני צבעים עם אותו שם — כל צבע צריך שם ייחודי למוצר.",
      nameRequired: "יש להזין שם מוצר",
      titlePrefix: "פרטים —",
      stockLabel: "מלאי:",
      productNameLabel: "שם המוצר",
      productDescLabel: "תיאור המוצר",
      englishContentTitle: "🌐 תוכן באנגלית",
      productNameEnLabel: "שם המוצר (אנגלית)",
      productDescEnLabel: "תיאור המוצר (אנגלית)",
      changeImage: "🖼️ החלף תמונה",
      priceLabel: "מחיר (₪)",
      lockedDuringSale: "נעול בזמן מבצע — נשלט לפי אחוז ההנחה למטה",
      costLabel: "עלות מוצר (₪)",
      onSaleCheckbox: "🏷️ מוצר במבצע",
      discountPercentLabel: "אחוז הנחה (%)",
      priceInfo: "המחיר הרגיל (האמיתי, לפני המבצע): ₪{regular} ← מחיר המבצע: ₪{sale}",
      autoRevertNote: "ⓘ ברגע שתבטלי את המבצע, המחיר יחזור אוטומטית ל-₪{regular}.",
      trendingCheckbox: "🔥 מוצר טרנדי",
      minStockLabel: "מינימום להתראה",
      seasonLabel: "עונה",
      generalStockLabel: "מלאי כללי",
      editByColorSize: "עריכת כמות לפי צבע/מידה (אופציונלי)",
      variantTotal: 'סה״כ: {total} יח׳',
      colorNamePlaceholder: "שם הצבע",
      colorNameEnPlaceholder: "שם הצבע (אנגלית)",
      sizeNameEnLabel: "שם המידה באנגלית (למוצרי אביזרים במידה אחידה)",
      addColorBreakdown: "➕ הוספת פילוח לפי צבע",
      saveButton: "שמירה 💾",
    },
    alerts: {
      title: "התראות מערכת",
      subtitle: "כל הסוגים — מלאי, ביקוש והזמנות. נשמרות לשבוע ומתעדכנות לפי מצב המוצרים",
      tabAll: "🔔 הכל",
      tabOutOfStock: "🚫 אזל מהמלאי",
      tabLowStock: "⚠️ מלאי נמוך",
      tabHighDemand: "🔥 ביקוש גבוה",
      tabCustomSize: "🔍 מידה מיוחדת",
      greaterThan15: "גדול מ־15",
      greaterThan30: "גדול מ־30",
      greaterThan60: "גדול מ־60",
      total: 'סה״כ:',
      requestsSuffix: "בקשות",
      viewRequestsButton: "📋 צפה בבקשות",
      noAlertsInFilter: "✅ אין התראות בסינון הנוכחי",
      outOfStockTitle: "🚫 המוצר אזל מהמלאי",
      lowStockTitle: "⚠️ מלאי נמוך",
      lowStockMsg: "{name} — נותרו {stock} יח׳",
      highDemandTitle: "🔥 ביקושים גבוהים",
      highDemandMsg: "{name} — נרשמו {count} בקשות",
      customSizeTitle: "🔍 בקשת מידה מיוחדת",
      customSizeMsg: 'הזמנה {orderId} — {name} · מידה: "{size}" — דורש אישור ידני',
      overdueShippingTitle: "⏰ משלוח מתעכב",
      overdueShippingMsg: "הזמנה {orderId} עברה את זמן המשלוח המובטח — כדאי לבדוק את הסטטוס",
    },
    dialogs: {
      barcodeNotFound: '⚠️ מוצר עם הקוד "{code}" לא נמצא במלאי',
      confirmLogout: "להתנתק?",
      confirmGoHome: "לחזור לדף הבית?",
    },
    loginOverlay: {
      title: "👑 כניסת מנהל ראשי",
      username: "שם משתמש",
      password: "סיסמה",
      showPassword: "הצג סיסמה",   
      hidePassword: "הסתר סיסמה",
      wrongCredentials: "❌ שם משתמש או סיסמה שגויים",
      forgotPasswordLink: "שכחת סיסמה?",
      forgotPasswordSending: "שולח קישור לאיפוס...",
      forgotPasswordSent: "✅ נשלח קישור לאיפוס סיסמה לכתובת המנהלת.",
      forgotPasswordFailed: "⚠️ לא הצלחנו לשלוח את הקישור. אפשר לנסות שוב בעוד רגע.",
      loginButton: "כניסה",
    },
    promo: {
      successTitle: "הפעולה הצליחה!",
      productPromoted: "\"{name}\" עודכן בדף הבית.",
      promotionCancelled: "הקידום בוטל בהצלחה.",
    },
    inventory: {
      title: " ניהול מלאי",
      filterToggle: "סינון",
      clearFilters: "אפס סינונים",

      filters: {
        category: "קטגוריה",
        gender: "מגדר",
        stockStatus: " סטטוס מלאי",
        productName: "שם מוצר",
        productCode: "קוד מוצר",
        promoOnly: "מוצרים לפרסום בלבד",
      },

      placeholders: {
        productName: "חיפוש לפי שם...",
        productCode: "לדוגמה: FS-001",
      },

      table: {
        image: "תמונה",
        code: "קוד",
        name: "שם מוצר",
        season: "עונה",
        stock: "מלאי",
        price: "מחיר",
        min: "מינימום",
        status: "סטטוס",
        actions: "פעולות",
      },

      summary: {
        units: "יחידות",
        products: "מוצרים",
      },

      options: {
        allCategories: "כל הקטגוריות",
        genders: {
          men: "גברים",
          women: "נשים",
          kids: "ילדים",
          unisex: "יוניסקס",
        },
        stockStatus: {
          available: "זמין",
          low: "נמוך",
          out: "אזל",
        },
      },

      buttons: {
        promote: "📢 פרסם",
        promoted: "✅ בפרסום",
        details: "פרטים",
        delete: "🗑️",
      },

      messages: {
        confirmDelete: "למחוק מוצר?",
        noResults: "אין מוצרים שמתאימים לסינון שנבחר",
      },

      badges: {
        available: "זמין",
        low: "נמוך",
        out: "אזל",
      },

      seasons: {
        summer: "קיץ",
        winter: "חורף",
        springFall: "אביב/סתיו",
        allSeasons: "כל העונות",
      },
    },

  },

  categoryLabels: {
    "חולצות": "חולצות",
    "מכנסיים": "מכנסיים",
    "שמלות": "שמלות",
    "עליוניות": "עליוניות",
    "נעליים": "נעליים",
    "אביזרים": "אביזרים",
  },

  genderLabels: {
    "נשים": "נשים",
    "גברים": "גברים",
    "ילדים": "ילדים",
    "יוניסקס": "יוניסקס",
  },

  orderStatusLabels: ["התקבלה", "בהכנה", "נשלחה", "נמסרה"],
  pickupStatusLabels: ["התקבלה", "בהכנה", "מוכן לאיסוף", "נאסף"],

  monthNames: [
    "ינואר", "פברואר", "מרץ", "אפריל", "מאי", "יוני",
    "יולי", "אוגוסט", "ספטמבר", "אוקטובר", "נובמבר", "דצמבר",
  ],

  customer: {
    chat: {
      pageTitle: "💬 צ'אטבוט FashionSync",
      pageSub: "שאל אותנו כל שאלה על החנות, מוצרים ומבצעים",
      botName: "SYNC – עוזר החנות",
      onlineStatus: "● זמין ומוכן לעזור",
      exitFullscreen: "צא ממסך מלא",
      enterFullscreen: "הרחב למסך מלא",
      generatedImageAlt: "המחשת לוק שנוצרה על ידי SYNC",
      outfitItemsTitle: "הפריטים בלוק",
      matchingProductsTitle: "מוצרים מתאימים",
      attachImageTitle: "שליחת תמונה",
      messagePlaceholder: "הודעה...",
      messageTooLong: "ההודעה ארוכה מדי. אפשר לשלוח עד {max} תווים — כדאי לקצר ולשלוח שוב.",
      serviceUnavailable: "העוזר אינו זמין כרגע. אפשר לנסות שוב בעוד רגע, או לעיין בעמוד המדיניות.",
      outfitImageReady: "הנה המחשת הלוק שביקשת:",
      requestTimedOut: "לא הצלחתי להשיב הפעם. אפשר לנסות שוב או לשאול משהו אחר.",
      welcomeMessage: "שלום! 👋 אני SYNC, העוזר החכם של FashionSync.<br />אני יכול לעזור לך למצוא בגדים, לבדוק מחירים ושעות פעילות, וגם ליצור לוק מלא שמתאים לאירוע שלך.<br />ייתכן שלפעמים אטעה.<br />איך אפשר לעזור לך היום?",
    },
    browse: {
      searchPlaceholder: "חיפוש לפי שם/ברקוד",
      refreshTitle: "🔄 רענן",
      scanTooltip: "סריקת ברקוד",
      genderAll: "לכולם",
      allCategories: "כל הקטגוריות",
      allSeasons: "כל העונות",
      seasonSummer: "☀️ קיץ",
      seasonWinter: "❄️ חורף",
      seasonSpringAutumn: "🌸 אביב / סתיו",
      allPrices: "כל המחירים",
      priceUnder150: "עד ₪150",
      price150to300: "₪150–₪300",
      price300to500: "₪300–₪500",
      priceOver500: "מעל ₪500",
      cart: "סל",
      pageTitle: "🏬 קטלוג מוצרים",
      guestBannerPart1: "גלישה כ",
      guestBannerGuestWord: "אורח",
      guestBannerPart2: ". ",
      guestBannerLoginWord: "כניסה",
      guestBannerPart3: " כדי להוסיף לסל ולהזמין.",
      loginButton: "🔑 כניסה",
      tabAll: "הכל",
      tabTrending: "🔥 טרנדים",
      tabBestsellers: "⭐ נמכרים ביותר",
      tabSale: "🏷️ מבצעים",
      noResults: "לא נמצאו פריטים לעונה או לסינון שבחרת.",
      seasonBannerSummer: "עכשיו קיץ! מוצגים פריטי הקיץ",
      seasonBannerWinter: "עכשיו חורף! מוצגים פריטי החורף",
      seasonBannerSpringAutumn: "עכשיו אביב/סתיו!",
      neutralSummer: "מוצגים פריטי הקיץ",
      neutralWinter: "מוצגים פריטי החורף",
      neutralSpringAutumn: "מוצגים פריטי אביב/סתיו",
    },

    productModal: {
      seasonSummer: "קיץ",
      seasonWinter: "חורף",
      seasonSpringAutumn: "אביב / סתיו",
      seasonAllYear: "כל השנה",
      colorLabel: "צבע",
      sizeLabel: "מידה *",
      selectSizePlaceholder: "מידה...",
      outOfStockSuffix: " (אזל מהמלאי)",
      otherSizeValue: "אחר",
      customSizeNumberPlaceholder: "מידה (מספר)…",
      customSizeLetterPlaceholder: "מידה (אותיות)…",
      duplicateSizeWarning: "המידה הזו כבר קיימת ברשימה — אפשר לבחור אותה ישירות במקום.",
      selectSizeBeforeAdd: "יש לבחור מידה לפני ההוספה לסל",
      guestAddToCart: "🔒 כניסה",
      tryOn: "📷 נסה עליי",
      addToCart: "🛒 הוספה לסל",
      notifyMe: "🔔 הודע לי",
      close: "סגירה",
    },

    productCard: {
      outOfStock: "מלאי אזל",
      lowStock: "מלאי נמוך",
      available: "זמין",
      login: "כניסה",
      colorsLabel: "צבעים",
      sizesLabel: "מידות",
      guestAddToCart: "🔒 הוספה לסל",
      inCartButton: "✓ בסל ({qty}) – להוסיף עוד",
      addToCart: "🛒 הוספה לסל",
      tryOn: "נסה עליי",
      notifyWhenBack: "🔔 הודע לי שיחזור למלאי",
      trending: "🔥 טרנד",
      bestseller: "⭐ נמכר ביותר",
    },

    cart: {
      title: "🛒 סל הקניות",
      empty: "אין פריטים בעגלה",
      earnPoints: "⭐ תצבור {points} נקודות על הזמנה זו",
      couponPlaceholder: "קוד קופון...",
      apply: "החל",
      discountApplied: "✅ הנחה הוחלה:",
      pointsPlaceholder: "כמה נקודות להשתמש? (יש לך {points})",
      usePoints: "השתמש",
      pointsRate: "כל 20 נק' = ₪1 — ",
      pointsWorth: "זה שווה ₪{amount} הנחה",
      pointsAvailable: 'סה"כ יש לך ₪{amount} זמינים',
      pointsRedeemed: "✅ נוצלו {points} נק' = ₪{amount} הנחה",
      totalLabel: 'סה"כ לתשלום:',
      checkout: "✅ מעבר לתשלום",
      giftCardRedeemPlaceholder: "קוד כרטיס מתנה (GC-... או RTN-...)",
    },

    checkout: {
      secureBadge: "🔒 תשלום מאובטח",
      sameDayDelivery: "⚡ אספקה עד הערב!",
      pickupDelivery: "המועד לאיסוף ייקבע בהמשך — לאחר שההזמנה תהיה מוכנה, נשלח אליך מייל לבחירת יום ושעה נוחים.",
      estimatedDeliveryPrefix: "זמן אספקה משוער:",
      stepDetails: "פרטים",
      stepShipping: "משלוח",
      stepPayment: "תשלום",
      stepConfirm: "אישור",

      step1: {
        personalDetailsTitle: "👤 פרטים אישיים",
        firstName: "שם פרטי",
        lastName: "שם משפחה",
        requiredField: "שדה חובה",
        email: "כתובת אימייל",
        invalidEmail: "אימייל לא תקין",
        phone: "טלפון",
        invalidPhone: "מספר לא תקין",
        shippingAddressTitle: "📦 כתובת למשלוח",
        street: "רחוב ומספר",
        city: "עיר",
        zip: "מיקוד",
        notes: "הערות למשלוח (אופציונלי)",
        continueButton: "מעבר לבחירת משלוח ←",
      },

      step2: {
        chooseShippingTitle: "🚚 בחירת שיטת משלוח",
        free: "חינם",
        deliveryEstimateTitle: "📅 תאריך אספקה משוער",
        backButton: "→ חזרה",
        continueButton: "מעבר לתשלום ←",
      },

      priceBox: {
        subtotal: "סכום ביניים",
        discount: "הנחה",
        pointsDiscount: "הנחת נקודות נאמנות",
        shipping: "משלוח",
        freeShipping: "חינם ✨",
        installments: "תשלומים",
        total: 'סה"כ לתשלום',
        giftCardDiscount: "זיכוי כרטיס מתנה",
      },

      step3: {
        paymentMethodTitle: "💳 שיטת תשלום",
        creditCard: "כרטיס אשראי",
        cashOnPickup: "מזומן באיסוף עצמי",
        cashToCourier: "מזומן לשליח",
        giftCard: "כרטיס מתנה",
        cardNumber: "מספר כרטיס",
        invalidCardNumber: "מספר כרטיס לא תקין",
        cardHolder: "שם בעל הכרטיס",
        requiredField: "שדה חובה",
        cardHolderId: "תעודת זהות של בעל הכרטיס",
        invalidCardId: "מספר ת.ז. לא תקין (9 ספרות)",
        expiry: "תוקף",
        invalidExpiry: "תוקף לא תקין",
        invalidCvv: "CVV לא תקין",
        installmentsCount: "מספר תשלומים",
        onePayment: "תשלום אחד",
        bitInfo: '📱 לאחר לחיצה על "שלם" תועבר לאפליקציית Bit לאישור.',
        bitPhone: "מספר טלפון (חשבון Bit)",
        invalidPhone: "מספר טלפון לא תקין",
        paypalInfo: '🅿️ לאחר לחיצה על "שלם" תועבר ל-PayPal לביצוע התשלום.',
        paypalEmailRequired: "כדי להמשיך עם PayPal צריך אימייל תקין בשלב הפרטים",
        cashInfoLine1: "💵 תשלום במזומן יבוצע בעת איסוף הסחורה מהחנות.",
        cashInfoLine1Courier: "💵 תשלום במזומן יבוצע לשליח בעת מסירת ההזמנה.",
        cashInfoLine2Bold: "",
        cashInfoLine2Rest: "",
        cashInfoLine2Prefix: "ההזמנה תישמר עבורך עד למועד האיסוף שייקבע.",
        giftCardCode: "קוד כרטיס מתנה",
        invalidGiftCard: "קוד לא תקין, כרטיס מנוצל, או שהיתרה נמוכה מסכום ההזמנה",
        termsAgreement: "קראתי ואני מסכים/ה לתנאי השימוש ולמדיניות הפרטיות.",
        mustAcceptTerms: "יש לאשר את התנאים להמשך",
        backButton: "← חזרה",
        payNow: "🔒 שלם עכשיו",
        trustSsl: "🔒 SSL מוצפן",
        trustPci: "🛡️ PCI DSS",
        trustReturns: "↩️ החזרות חינם 30 יום",
        trustSupport: "📞 תמיכה 24/7",
        applyGiftCardButton: "החל",   
        removeButton: "הסר",          
        giftCardPartialNote: "אם היתרה לא מכסה את כל הסכום, ניתן עדיין לבחור אמצעי תשלום נוסף בהמשך להשלמת התשלום.",
        fullyCoveredNote: "✅ הסכום מכוסה במלואו על ידי הזיכויים שהוחלו — אין צורך באמצעי תשלום נוסף.",
      },

      step4: {
        titleGiftCard: "כרטיס המתנה נרכש בהצלחה!",
        titleCash: "ההזמנה נשמרה!",
        titleDefault: "ההזמנה בוצעה!",
        subGiftCard: "פרטי הרכישה קשורים לחשבון:",
        subCash: "הזמנתך נשמרה בהצלחה.",
        subDefault: "אישור הזמנה נשלח לאימייל:",
        cashPaymentNote: "תשלום יבוצע בעת האיסוף מהחנות",
        receiptNumberPurchase: "🧾 מספר רכישה:",
        receiptNumberOrder: "🧾 מספר הזמנה:",
        orderDetailsTitle: "📋 פרטי ההזמנה",
        giftCardCodeLabel: "🎁 קוד כרטיס המתנה:",
        payInStore: "לתשלום בחנות",
        totalPaid: 'סה"כ ששולם',
        backToStore: "→ חזרה לחנות",
        printReceipt: "🖨️ הדפס קבלה",
      },
    },

    processing: {
      text: "מעבד תשלום...",
      sub: "אנא המתן, אל תסגור את הדף",
    },

    orders: {
      title: "📦 ההזמנות שלי",
      subtitle: "מעקב אחרי הזמנות ומשלוחים",
      noDate: "ללא תאריך",
      allMonths: "📅 כל החודשים",
      allFilter: "הכל",
      noOrdersInStatus: "אין הזמנות במצב הזה",
      withReturnsFilter: "עם בקשת החזרה",
      cancelledFilter: "בוטלו",
      pickupFilter: "איסוף עצמי",
      cancelledLabel: "ההזמנה בוטלה",
      cancelOrderButton: "ביטול הזמנה",
      confirmCancelOrder: "לבטל את ההזמנה?",
      cancelSuccess: "✅ ההזמנה בוטלה בהצלחה.",
      pickupReadyTitle: "ההזמנה מוכנה לאיסוף — לבחירת מועד",
      pickupConfirmButton: "אישור מועד",
      pickupSavingButton: "⏳ שומר...",
      pickupScheduledLabel: "מועד איסוף שנקבע:",
      pickupErrorRequired: "נא לבחור תאריך ושעה",
      pickupErrorFormat: "פורמט שעה לא תקין (לדוגמה 14:00)",
      pickupErrorClosed: "החנות סגורה בתאריך שנבחר",
      pickupErrorOutsideHours: "השעה חייבת להיות בין {open} ל-{close}",
    },

    wishlist: {
      title: "❤️ רשימת הבקשות שלי",
      subtitle: "פריטים ששמרת לאחר כך",
      empty: "רשימת הבקשות ריקה.",
    },

    loyalty: {
      title: "⭐ נקודות וקופונים",
      subtitle: "הטבות בלעדיות עבורך",
      myPoints: "הנקודות שלי",
      pointsAbbrev: "נק'",
      redemptionSuffix: "לשימוש בקנייה הבאה",
      myCoupons: "הקופונים שלי",
      noActiveCoupons: "אין כרגע קופונים פעילים",
      discountSuffix: "% הנחה",
      onEntirePurchase: "על כל הקנייה",
      copy: "העתק",
      seasonSummerOnly: "בקיץ בלבד",
      seasonWinterOnly: "בחורף בלבד",
      seasonSpringAutumnOnly: "באביב/סתיו בלבד",
    },

    policy: {
      title: "📋 מדיניות והחזרות",
      subtitle: "כל המידע שאתה צריך לדעת",
      aboutStoreTitle: "🏬 אודות החנות",
      aboutStoreText: "החנות שלנו פועלת באופן מקוון (אונליין) בלבד. הכתובת {address} מיועדת לאיסוף עצמי מתואם מראש בלבד — אין בה חנות פיזית לקבלת קהל.",
      returnsTitle: "🔄 מדיניות החזרות",
      returnsText:
        "ניתן לבקש החזרה עבור פריטים שנמסרו, דרך עמוד \"ההזמנות שלי\" — לפי כל פריט בנפרד, עד {days} ימים בלבד ממועד המסירה. הבקשה נבדקת על ידי הצוות שלנו, ותקבל/י עדכון באתר ובמייל האם היא אושרה או נדחתה. במקרה של אישור, יינתן זיכוי בכרטיס מתנה בסכום הפריט שהוחזר, לשימוש בקנייה הבאה.",
      cancellationTitle: "❌ מדיניות ביטול הזמנה",
      cancellationText:
        "ניתן לבטל הזמנה לגמרי ובאופן מיידי, ללא צורך באישור מהצוות שלנו, עד {hours} שעות בלבד מרגע ביצוע ההזמנה — דרך עמוד \"ההזמנות שלי\". לאחר {hours} שעות אפשרות הביטול אינה זמינה עוד.",
      shippingTitle: "🚚 מדיניות משלוחים",
      shippingLine1: "משלוח רגיל: 5–7 ימי עסקים — ₪25 (חינם לרכישות מעל ₪{threshold})",
      shippingLine2: "משלוח מהיר: 2–3 ימי עסקים — ₪29",
      shippingLine3: "משלוח באותו יום: עד 23:59 היום — ₪59 (מרכז בלבד)",
      shippingLine4: "איסוף עצמי: מחר 10:00–20:00, חינם — {address}",
      privacyTitle: "🔒 פרטיות ואבטחה",
      privacyLine1: "אנחנו לא שומרים פרטי כרטיס אשראי. כל העסקאות מוצפנות.",
      contactTitle: "📞 יצירת קשר",
      contactEmailPrefix: "✉️ מייל:",
      contactPhonePrefix: "📞 טלפון:",
      hoursPrefix: "🕒 שעות פעילות:",
      contactEmail: "אימייל: support@fashionsync.co.il",
      contactNameLabel: "שם",
      contactEmailFromAccount: "הכתובת נלקחת מהחשבון שלך",
      contactEmailLabel: "כתובת מייל לחזרה אליך",
      contactMessageLabel: "הודעה",
      contactMessagePlaceholder: "איך אפשר לעזור?",
      contactSubmitButton: "שליחת הודעה",
      contactSubmitSuccess: "✅ ההודעה נשלחה בהצלחה! נחזור אליך בהקדם.",
    },

    giftCard: {
      title: "🎁 כרטיס מתנה",
      subtitle: "שלח מתנה לאהובים שלך",
      createTitle: "צור כרטיס מתנה",
      amountLabel: "סכום",
      otherAmount: "אחר...",
      enterAmountPlaceholder: "סכום...",
      recipientNameLabel: "שם המקבל",
      namePlaceholder: "שם...",
      personalMessageLabel: "הודעה אישית",
      messagePlaceholder: "ברכה...",
      buyButton: "🎁 רכוש כרטיס מתנה",
      previewTitle: "תצוגה מקדימה",
      giftCardLabel: "כרטיס מתנה",
      errorRecipientRequired: "נא להזין שם מקבל.",
      errorInvalidAmount: "נא להזין סכום תקין (מינימום ₪10).",
      errorLoginRequired: "יש להתחבר כדי לרכוש כרטיס מתנה.",
      forLabel: "עבור:",
      checkBalanceTitle: "💳 בדיקת יתרה בכרטיס מתנה",
      checkCodePlaceholder: "קוד כרטיס מתנה (GC-...)",
      checkBalanceButton: "בדיקת יתרה",
      currentBalance: "יתרה נוכחית:",
      outOf: "מתוך",
      statusLabel: "סטטוס:",
      statusUsed: "נוצל במלואו",
      statusActive: "פעיל",
      statusPending: "ממתין לאישור",
      statusRejected: "נדחה",
    },

    dialogs: {
      outOfStockSelection: "מצטערים, הצבע/מידה שבחרת אזלו מהמלאי",
      invalidCoupon: "קוד קופון לא תקין.",
      couponSeasonOnly: "קוד הקופון תקף רק בעונה המתאימה לו.",
      invalidPointsInput: "יש להזין מספר נקודות תקין",
      insufficientPoints: "אין לך מספיק נקודות. יש לך {points} נקודות זמינות.",
      cartAlreadyCovered: "הסכום בעגלה כבר מכוסה, אין צורך בנקודות נוספות.",
      emptyCart: "הסל ריק",
      shareNotSupported: "הדפדפן לא תומך בשיתוף ישיר, לכן התמונה נשמרה למחשב.",
      shareNotSupportedBrowser: "השיתוף לא נתמך בדפדפן הזה.",
      notifyConfirmMessage: "נשלח לך מייל לכתובת {email} ונציג באנר בראש החנות כש{name} יחזור למלאי. להמשיך?",
      notifyAlreadyRegistered: "כבר נרשמת להתראה על המוצר הזה. נודיע לך ברגע שיחזור למלאי.",
      notifySuccessMessage: "נרשמת בהצלחה! נעדכן אותך במייל ({email}) ובאנר בראש החנות כשהמוצר יחזור למלאי.",
      tryOnErrorUploadImage: "יש להעלות תמונה לפני הפעלת נסה עליי",
      tryOnErrorProductNotFound: "המוצר שנבחר לא נמצא",
      tryOnErrorProductImageMissing: "למוצר שנבחר אין תמונה, ולכן אי אפשר להפעיל נסה עליי",
      tryOnErrorGeneric: "אירעה שגיאה בהפעלת נסה עליי",
      barcodeNotFound: '⚠️ מוצר עם הקוד "{code}" לא נמצא',
      confirmGoHome: "לחזור לדף הבית?",
      confirmGuestAction: "לפעולה זו עליך להתחבר.\nלעבור לדף הכניסה?",
      confirmLogout: "להתנתק?",
      orderSaveError: "אירעה שגיאה בשמירת ההזמנה: {message}",
      unknownError: "שגיאה לא ידועה",
    },

    returns: {
      requestButton: "🔄 בקש החזרה",
      returnWindowExpired: "חלון ההחזרה ({days} ימים) הסתיים",
      alreadyRequested: "🔄 בקשת החזרה נשלחה",
      statusApproved: "✅ ההחזרה אושרה",
      statusRejected: "❌ ההחזרה נדחתה",
      modalTitle: "בקשת החזרה / זיכוי",
      selectItemLabel: "פריט להחזרה",
      noItemsAvailable: "כל הפריטים בהזמנה זו כבר קיבלו בקשת החזרה.",
      reasonLabel: "סיבת ההחזרה",
      reasonDefective: "מוצר פגום",
      reasonWrongSize: "מידה לא מתאימה",
      reasonNotAsDescribed: "המוצר לא תואם לתיאור",
      reasonChangedMind: "שיניתי את דעתי",
      reasonOther: "אחר",
      noteLabel: "הערה נוספת (לא חובה)",
      notePlaceholder: "פרטים נוספים...",
      submitButton: "שליחת בקשה",
      submitSuccess: "✅ הבקשה נשלחה בהצלחה! נעדכן אותך כשתטופל.",
      updateBannerApproved: "🎉 בקשת ההחזרה עבור {name} אושרה!",
      updateBannerRejected: "בקשת ההחזרה עבור {name} לא אושרה",
    },

    shareModal: {
      title: "📤 שתף פריט",
      whatsapp: "WhatsApp",
      email: "אימייל",
      copyLink: "העתק קישור",
      copiedMessage: "✅ הקישור הועתק!",
    },

    tryOn: {
      title: "📸 נסה עליי",
      uploadSectionTitle: "🤳 העלאת תמונה",
      uploadText: "העלאת תמונה",
      uploadedAlt: "תמונה שהועלתה",
      changeImage: "🔁 החלף תמונה",
      deleteImage: "🗑️ מחיקת תמונה",
      processingButton: "⏳ מעבד תמונה...",
      activateButton: "✨ הפעלת נסה עליי",
      resultTitle: "🪞 התוצאה",
      generatingResult: "⏳ יוצר את תמונת ה־Try-On...",
      resultAlt: "תוצאת נסה עליי",
      saveImage: "💾 שמירת תמונה",
      shareImage: "🔗 שתף",
      emptyResultPlaceholder: "יש להעלות תמונה וללחוץ על „הפעלת נסה עליי”",
      shareTitleNative: "FashionSync - נסה עליי",
      shareTextNative: "שיתוף תמונת נסה עליי",
    },

    misc: {
      shareMessageTemplate: "בדקו את {name} ב-FashionSync – ₪{price}",
      shareEmailSubjectPrefix: "ראה את ",
      copiedButtonText: "✓ הועתק",
      stockBackBannerText: "🎉 {name} חזר למלאי!",
      gotItButton: "✕ הבנתי",
      guestFallbackName: "אורח",
      giftCheckErrorEmptyCode: "נא להזין קוד כרטיס מתנה",
      giftCheckErrorNotFound: "קוד כרטיס מתנה לא נמצא",
      discountSuffix: "% הנחה",
    },

    preCheckoutFeedback: {
      title: "איך הייתה חוויית הקנייה?",
      textPlaceholder: "מה נוכל לשפר?",
      submitButton: "מעבר לתשלום ←",
      optionalHint: "אפשר להמשיך גם בלי לדרג",
      backToCart: "חזרה לסל",
    },

    sidebar: {
      roleBadge: "לקוח",
      guestModeLabel: "👀 מצב צפייה בלבד",
      guestLoginButton: "🔑 כניסה לגישה מלאה",
      navChat: "צ'אטבוט",
      navCatalog: "קטלוג",
      navWishlist: "רשימת בקשות",
      navOrders: "ההזמנות שלי",
      navLoyalty: "נקודות וקופונים",
      navGiftCard: "כרטיס מתנה",
      navPolicy: "מדיניות",
      lightMode: "מצב בהיר",
      darkMode: "מצב כהה",
      logout: "התנתק",
      menuAriaLabel: "תפריט",
      backAriaLabel: "חזרה",      
      chatAriaLabel: "צ'אטבוט",
      cartAriaLabel: "עגלת קניות",
    },
  },

  home: {
    navbar: {
      tagline: "מערכת ניהול חנות בגדים",
      themeAriaLabel: "מצב כהה/בהיר",
      lightMode: "מצב בהיר",
      darkMode: "מצב כהה",
    },
    hero: {
      welcomeTo: "ברוך הבא ל",
      subtitle: "הסטייל שלך מחכה לך כאן",
      customerAreaQuote: "\"אזור הלקוחות ✨ הסטייל שלכם מתחיל כאן – התחברו והמשיכו לשופינג\"",
      loginButtonLabel: "כניסה עם מייל וסיסמה",
      loginButtonSub: "גישה מלאה להזמנות, מועדפים וסל הקניות שלכם",
      browseButtonLabel: "רק רוצים לראות?",
      browseButtonSub: "צפו בקולקציה ללא התחברות",
    },
    footer: {
      copyright: "FashionSync · כל הזכויות שמורות · 2026",
      managerLogin: "כניסת מנהל",
    },
    loginModal: {
      title: "כניסה לחנות 🔑",
      close: "סגירה",
      hintBold: "כניסה מותרת רק עם Gmail",
      hintRest: ' — בפעם הראשונה זה "רישום", ובפעמים הבאות חייבים אותה סיסמה 😊',
      emailLabel: "כתובת אימייל",
      passwordLabel: "סיסמה",
      showPassword: "הצג סיסמה",   
      hidePassword: "הסתר סיסמה",
      passwordPlaceholder: "סיסמה (לפחות 8 תווים)",
      loggingIn: "מתחברת...",
      loginButton: "כניסה",
      forgotPasswordLink: "שכחת סיסמה?",
      forgotPasswordSending: "שולחת קישור לאיפוס...",
      forgotPasswordSent: "✅ אם קיים חשבון עם המייל הזה, נשלח אליו קישור לאיפוס סיסמה.",
    },
    authErrors: {
      wrongPassword: "אימייל או סיסמה שגויים",
      emailInUse: "כבר קיים חשבון עם האימייל הזה",
      weakPassword: "הסיסמה חייבת להכיל לפחות 8 תווים",
      invalidEmail: "כתובת אימייל לא תקינה",
      tooManyRequests: "יותר מדי ניסיונות. אפשר לנסות שוב מאוחר יותר",
      genericError: "אירעה שגיאה. אפשר לנסות שוב",
      invalidGmail: "אימייל לא תקין — חייב להיות ‎@gmail.com",
      passwordTooShort: "הסיסמה חייבת להכיל לפחות 8 תווים",
    },
    verification: {
      title: "🔐 אימות כתובת המייל",
      subtitle: "שלחנו קוד בן 6 ספרות לכתובת {email}. יש להזין אותו כדי להשלים את ההרשמה.",
      codeLabel: "קוד אימות",
      codePlaceholder: "000000",
      verifyButton: "אמת קוד",
      verifying: "מאמתת...",
      resendButton: "שליחת קוד חדש",
      resendCooldown: "אפשר לשלוח קוד נוסף בעוד {seconds} שניות",
      resendSuccess: "✅ קוד חדש נשלח",
      errorNotFound: "לא נמצאה בקשת אימות. אפשר לבקש קוד חדש.",
      errorExpired: "הקוד פג תוקף. בקשי קוד חדש.",
      errorMismatch: "קוד שגוי. אפשר לנסות שוב.",
      didntReceiveHint: "לא קיבלת? כדאי לבדוק בתיקיית הספאם ואז לבקש קוד חדש.",
      close: "סגירה",
      likelyFakeEmailHint: "לא קיבלת? כדאי לבדוק בתיקיית הספאם, או לוודא שהכתובת נכונה.",
      errorSuperseded: "שלחנו קוד חדש — השתמשי בו, הקוד הקודם כבר אינו בתוקף.",
      errorTooManyRequests: "נשלחו יותר מדי קודים לכתובת הזו. אפשר לנסות שוב בעוד שעה.",
      errorResendFailed: "לא הצלחנו לשלוח קוד חדש. אפשר לנסות שוב בעוד רגע.",
    },
  },

  shippingOptionLabels: {
    standard: {
      label: "משלוח רגיל",
      days: "5–7 ימי עסקים",
      note: "חינם לרכישות מעל ₪{threshold}",
    },
    express: {
      label: "משלוח מהיר",
      days: "2–3 ימי עסקים",
      note: "",
    },
    same_day: {
      label: "משלוח באותו יום",
      days: "עד 23:59 היום",
      note: "מרכז בלבד",
    },
    pickup: {
      label: "איסוף עצמי",
      days: "מועד ייקבע לאחר אישור ההזמנה",
      note: "",
    },
  },
};